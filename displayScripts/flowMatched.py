...
def advanceTime(dt=1):
    ...
    for p in pList:
        ...
        # Limit speed
        if (math.sqrt(vx**2 + vy**2) > p.maxSpeed):
...
def changeFields():
    for f in range(6):    
        for i in range(64):
            for j in range(64):
                vField[i,j,f,0] = 0.2*(random.random()*2-1) #0.001*(j-32)
                vField[i,j,f,1] = 0.2*(random.random()*2-1) #0.001*(i-32)               
        sigma = 10
    ...
    for f in range(6):    
        for i in range(64):
            for j in range(64):
                if f in (0,1,2,3):
                    vField[i,j,f,0] +=  (1-2*(f%2))*0.001*(j-32)
                    vField[i,j,f,1] +=  (1-2*(f%2))*0.001*(i-32)
                elif f == 4:
                    vField[i,j,f,0] +=  -0.001*(j-32)
                    vField[i,j,f,1] +=  -0.001*(i-32)
                else: #f == 5
                    vField[i,j,f,0] +=  -0.001*(j-32)
                    vField[i,j,f,1] +=  -0.001*(i-32)
...
for i in range(pNum):
    ...
    p.pos = [random.randint(0,63), random.randint(0,63)]
    p.face = random.randint(0,3)   
    p.maxSpeed = maxSpeed
    pList[i] = p
