...
# Rule [ruleS,ruleB,startDensity]
bloomBoom = [[3,4], [3], 0.5]
wildGrowth = [[3,2,7], [3], 0.18]
blobbs = [[3,5], [3,4], 0.18]
gol = [[2,3], [3], 0.32]

ruleList      = [gol, bloomBoom, wildGrowth, blobbs]
ruleListNames = ["gol", "bloomBoom", "wildGrowth", "blobbs"]
...
start_density = 0.50; # Possibility for each cell to be alive when world is seeded
max_pop = 6000; # Clear world and reseed when pop reaches cutoff
min_pop = 100; # Clear world and reseed when pop reaches cutoff
popcheck = 100; # Generation at whih population is checked
...
def randomRules():
    global ruleS, ruleB
    ruleNumber = random.randint(0,len(ruleList)-1)
    rule = ruleList[ruleNumber]
    ruleS = rule[0]
    ruleB = rule[1]
    start_density = rule[2]
    print("Rule: "+ruleListNames[ruleNumber])
...
while(1):
    ...
    # Sprinkle in some chaos by random extra seeding
    if(random.random() < 0.05):
        seed_world(start_density/2)
        #print("Sprinkles!!")
